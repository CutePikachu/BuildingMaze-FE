...
function findPath (graph, srcX, srcY, destX, destY) {
	...
	while(queue.length) {
		...
		if (tmp.x > 0 && that[tmp.x - 1][tmp.y] == false && graph[tmp.x - 1][tmp.y] != "w") {
	        that[tmp.x - 1][tmp.y] = true;
	        queue.push((tmp.x - 1) * 10 + tmp.y);
	    }
	    if (tmp.x < 9 && that[tmp.x + 1][tmp.y] == false && graph[tmp.x + 1][tmp.y] != "w") {
	        that[tmp.x + 1][tmp.y] = true;
	        queue.push((tmp.x + 1) * 10 + tmp.y);
	    }
	    if (tmp.y > 0 && that[tmp.x][tmp.y - 1] == false && graph[tmp.x][tmp.y - 1] != "w") {
	        that[tmp.x][tmp.y - 1] = true;
	        queue.push((tmp.x) * 10 + tmp.y - 1);
	    }
	    if (tmp.y < 9 && that[tmp.x][tmp.y + 1] == false && graph[tmp.x][tmp.y + 1] != "w") {
	        that[tmp.x][tmp.y + 1] = true;
	        queue.push((tmp.x) * 10 + tmp.y + 1);
	    }
